...
namespace zikwall\encore\modules\core\components\i18n;
...
class I18N extends \yii\i18n\I18N
{
     ...
     */
    public $messageOverwritePath = '@zikwall/encore/modules/core/messages';
     ...
     *
     * @param \zikwall\encore\modules\core\modules\user\models\User $user
     */
        ...
        foreach (Yii::$app->moduleManager->getModules(['includeCoreModules' => true, 'returnClass' => true]) as $moduleId => $className) {
            ...
            if (substr($category, 0, strlen($moduleCategory)) === $moduleCategory) {
                ...
                $this->translations[$moduleCategory . '*'] = [
                    'class' => 'zikwall\encore\modules\core\components\i18n\MessageSource',
            ...
            /*// search sub modules in this module
            foreach (Yii::$app->getModule($moduleId)->getModules() as $subModuleId => $subModuleClass){
                $subModuleCategory = $this->getTranslationCategory($subModuleId);

                if (substr($category, 0, strlen($subModuleCategory)) === $subModuleCategory) {
                    $reflector = new \ReflectionClass($subModuleClass);

                    $this->translations[$subModuleCategory . '*'] = [
                        'class' => 'zikwall\encore\modules\core\components\i18n\MessageSource',
                        'sourceLanguage' => Yii::$app->sourceLanguage,
                        'sourceCategory' => $subModuleCategory,
                        'basePath' => dirname($reflector->getFileName()) . '/messages',
                    ];
                }
            }*/
